    ...
    async with gagan.conversation(event.chat_id) as conv: 
        if not s:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="http://t.me/ogxmaster")]])
    ...
    for i in range(len(ids)):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/ogxmaster")]])
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="http://t.me/ogxmaster")]])
        except Exception as e:
            #print(e)
            logger.info(e)
            await client.send_message(sender, f"An error occurred during cloning, batch will continue.\n\n**Error:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/ogxmaster")]])
        n = i + 1
        if n == len(ids):
            return -2
...
TEXT = "👋 Hi, This is 'Paid Restricted Content Saver' bot Made with ❤️ by __**OG MASTER**__."
...
async def start_command(event):
    ...
    buttons = [
        [Button.inline("Cancel", data="cancel"),
         Button.inline("Cancel", data="cancel")],
        [Button.url("Join Channel", url="https://telegram.dog/ogxmaster")]
    ]
